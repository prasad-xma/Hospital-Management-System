...
const PendingReports = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await labService.byStatus('PENDING');
  ...
  const complete = async (id) => {
    try {
      const response = await labService.updateStatus(id, 'COMPLETED');
      if (response.success) {
            ...
            <tbody className="divide-y">
              {filtered.map(r => (
                <tr key={r.id}>
                  ...
                  <td className="px-4 py-3"><span className="bg-yellow-100 text-yellow-800 px-2 py-1 rounded text-xs">{r.status}</span></td>
                  <td className="px-4 py-3 space-x-3">
                    {r.fileUrl && (
                      <a href={`http://localhost:8084${r.fileUrl}`} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">View</a>
                    )}
                    <button onClick={()=>complete(r.id)} className="text-green-600 hover:underline">Mark Complete</button>
                  </td>
                </tr>
              ))}
            </tbody>
